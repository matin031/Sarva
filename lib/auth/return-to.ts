...
const ALLOWED_PREFIXES = [
  ...
  "/panel/classes",
  /* ⚠️ درسنامهٔ «ایهام» ورود می‌خواهد چون کاربر را به اسم صدا می‌زند.
     بدونِ این پیشوند، کسی که وسطِ درس نشستش تمام می‌شود، پس از ورود به صفحهٔ خانه پرت می‌شد. */
  "/learn",
  "/payment/result",
  /* ⚠️ اگر نشستِ کاربر وسطِ پرداخت از دست برود، بازگشت از درگاه باید *بعد از
     ورود* دوباره اجرا شود — با همان پارامترهای درگاه. وگرنه نتیجهٔ پرداخت
     فقط از مسیرِ «بررسی دوباره» روشن می‌شد. این مسیر خودش هیچ پارامتری را
     باور نمی‌کند (امضا را آداپتورِ درگاه می‌سنجد). */
  "/payment/return",
] as const;
