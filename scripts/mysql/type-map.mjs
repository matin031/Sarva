 ...
 *
 * ⚠️ و همین بند دیگر برقرار نیست، چون آن خانواده از collation ها کنار
 *    گذاشته شد. هاست این پروژه MariaDB است و utf8mb4_0900_* آنجا اصلاً
 *    وجود ندارد؛ هر CREATE TABLE با آن‌ها رد می‌شود.
 *
 *    جانشین‌ها (utf8mb4_bin و utf8mb4_unicode_ci) در هر دو موتور هستند ولی
 *    **PAD SPACE** اند، نه NO PAD. آزموده شد:
 *
 *        utf8mb4_bin        : 'a' = 'a '  →  درست
 *        utf8mb4_nopad_bin  : 'a' = 'a '  →  نادرست  (ولی فقط در MariaDB هست)
 *
 *    یعنی «سلام» و «سلام » از نظر قیدهای یکتایی یکی شمرده می‌شوند، در حالی
 *    که در Postgres دو چیز بودند.
 *
 *    این تفاوت آگاهانه پذیرفته شد: یکتایی *سخت‌گیرانه‌تر* می‌شود و نه
 *    شل‌تر — یعنی نهایتاً یک درجِ تکراریِ نزدیک رد می‌شود، که خودش رفتار
 *    بهتری است. هیچ کوئریِ این پروژه به تفاوتِ فاصلهٔ انتهایی تکیه ندارد.
 *
 *    آنچه *حفظ* شد و مهم‌تر بود: «ی» و «ي» در هر دو جانشین همچنان دو نویسهٔ
 *    متفاوت‌اند (آزموده شد).
 ...
 *
 * ⚠️ `utf8mb4_bin` و نه `utf8mb4_0900_as_cs`.
 *
 * دومی دقیق‌تر بود (مرتب‌سازیِ یونیکدی به‌جای مقایسهٔ نقطه‌کدی) ولی **فقط در
 * MySQL 8 وجود دارد**. هاست این پروژه MariaDB است و آنجا هر جدولی با آن
 * collation در همان دستور CREATE TABLE رد می‌شود.
 *
 * `utf8mb4_bin` در هر دو موتور هست و همان تضمینی را می‌دهد که به آن تکیه
 * داریم: بزرگی و کوچکی و اعراب هر دو معنادارند. تنها تفاوت، *ترتیبِ*
 * ORDER BY روی متن فارسی است — که هیچ کوئریِ این پروژه به آن وابسته نیست
 * (مرتب‌سازی‌ها بر اساس زمان و شمارنده‌اند، نه الفبا).
 */
export const TEXT_COLLATION = "utf8mb4_bin";

/**
 * فقط برای سه ستون ایمیل که در مبدأ citext اند.
 *
 * ⚠️ `utf8mb4_unicode_ci` و نه `utf8mb4_0900_as_ci` — به همان دلیلِ بالا:
 * آن یکی در MariaDB وجود ندارد.
 *
 * تفاوتِ معناییِ باقی‌مانده: unicode_ci علاوه بر بزرگی و کوچکی، به اعراب هم
 * حساس نیست. برای ایمیل بی‌اثر است (ایمیل‌های واقعی ascii اند) و همان چیزی
 * را که citext تضمین می‌کرد — یکتاییِ بی‌توجه به بزرگی و کوچکی — نگه می‌دارد.
 */
export const CITEXT_COLLATION = "utf8mb4_unicode_ci";
