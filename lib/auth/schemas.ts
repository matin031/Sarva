...
/**
 * شمارهٔ موبایل — همان قانونِ `lib/auth/phone.ts`، یک بار برای فرم و سرور.
 *
 * ⚠️ خروجیِ این فیلد **شکلِ متعارف** است (`989123456789`) و نه آنچه کاربر
 * تایپ کرده. دلیلش در `phone.ts` نوشته شده: اگر `09123456789` و
 * `+989123456789` هرکدام یک ردیف بسازند، «این شماره قبلاً ثبت شده؟» دیگر
 * پاسخِ قطعی ندارد. نرمال‌سازی همین‌جا اتفاق می‌افتد تا فرم و سرور دقیقاً یک
 * رشته را ببینند.
 *
 * ⚠️ و عمداً همان `normalizePhone` صدا زده می‌شود و نه یک regex محلی: آن
 * تابع ارقامِ فارسی، فاصله و خط تیره را هم می‌پذیرد. یک regexِ سرانگشتی
 * (`/^09\d{9}$/`) دانش‌آموزی را که با کیبورد فارسی «۰۹۱۲…» نوشته رد می‌کرد.
 */
export const phoneField = z
  .string()
  .trim()
  .min(1, "شمارهٔ موبایل را وارد کنید")
  .refine((v) => normalizePhone(v) !== null, "شمارهٔ موبایل معتبر نیست (مثل ۰۹۱۲۳۴۵۶۷۸۹)")
  // refine بالا تضمین کرده که null نیست؛ این فقط همان مقدار را برمی‌گرداند.
  .transform((v) => normalizePhone(v) as string);

/**
 * ⚠️ `nameField` (یک فیلدِ واحد برای کلِ نام) حذف شد.
 *
 * ثبت‌نام حالا **نام و نام خانوادگیِ جدا** می‌گیرد، به سه دلیل که هر سه
 * واقعی‌اند:
 *
 *   ۱. جدولِ `users` از مهاجرت ۰۰۹ همین دو ستون را دارد و `full_name` را یک
 *      تریگر از رویشان می‌سازد. فیلدِ واحد یعنی `first_name` و `last_name`
 *      برای هر ثبت‌نامِ ایمیلی خالی می‌ماندند — و گیتِ «حسابِ نیمه‌ساخته»
 *      (lib/auth/onboarding.ts) دقیقاً همان دو ستون را می‌خواند.
 *   ۲. «فعال‌سازی حساب دبیر» و کلاس‌ها هم به همان دو ستون تکیه دارند
 *      (`lib/teacher/requests.ts`).
 *   ۳. الگوی نام حالا از `lib/profile/schemas.ts` می‌آید — همان قانونی که
 *      فرمِ پروفایل و صفحهٔ تکمیلِ حساب استفاده می‌کنند. سه نسخهٔ جدا از
 *      «نامِ فارسی چیست» یعنی سه پیامِ خطای متفاوت برای یک ورودی، و
 *      نیم‌فاصله («حسین‌زاده») در یکی از آن سه مجاز نبود.
 */
...
/**
 * ثبت‌نام با **ایمیل** — نام، نام خانوادگی، ایمیل، رمز. و بس.
 *
 * ⚠️ شماره موبایل اینجا گرفته **نمی‌شود** و این عمدی است.
 *
 * یک روز گرفته شد و اشتباه بود: فرمِ ثبت‌نام دو تب دارد و تبِ «موبایل»
 * دقیقاً برای کسی است که می‌خواهد با شماره ثبت‌نام کند. گرفتنِ شماره در
 * تبِ «ایمیل» یعنی از کاربری که صریحاً ایمیل را انتخاب کرده، چیزی بخواهیم
 * که همان‌جا کنارش یک تبِ کامل برایش هست — و فرمِ پنج‌فیلدی بسازیم برای
 * کاری که سه فیلد می‌خواهد.
 *
 * کسی که با ایمیل ثبت‌نام کرده و بعداً شماره می‌خواهد، از «تنظیمات»
 * اضافه‌اش می‌کند (`/api/v1/auth/phone/send-code` و `phone/verify`) — و
 * آنجا شماره **تأیید** هم می‌شود، که از ثبتِ تأییدنشده در فرمِ ثبت‌نام
 * بهتر است.
 */
