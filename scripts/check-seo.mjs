...
for (const page of PAGES) {
  ...
  if (page.kind === "public") {
    ...
    /* لینکِ صفحه‌بندی باید در HTMLِ اولیه باشد، نه فقط با جاوااسکریپت.
       ...
       ⚠️ ولی فقط وقتی چیزی برای صفحه‌بندی هست. با دیتابیسِ خالی، صفحهٔ دوم
       طبیعتاً نه سروده‌ای دارد نه لینکی — و شکست دادنِ آزمون در آن حالت
       یعنی گزارشِ ایرادی که وجود ندارد. اولین بار دقیقاً همین شد: دادهٔ
       آزمایشی را پاک کردم و ابزار «ایراد» داد. */
    if (page.needsPageLink) {
      const feedHasItems = /class="[^"]*gc?-?poem|<article/i.test(r.html);
      if (!feedHasItems) {
        console.log("      ⓘ فهرست خالی است — آزمونِ لینکِ صفحه‌بندی رد شد");
      } else if (!/href="\/sarvaclub\?page=\d+"/.test(r.html)) {
