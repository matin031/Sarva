...
export interface PreparedStep {
  /**
   * هویتِ *این نوبت* — نه هویتِ پرسش.
   *
   * ⚠️ وقتی «اجازهٔ تکرارِ سؤال در همین دور» روشن است، یک پرسش می‌تواند چند
   * بار در `steps` بیاید. تا پیش از این، هم کلیدِ React و هم `tileId` از
   * `question.id` ساخته می‌شدند، پس دو مرحلهٔ متفاوت هویتِ یکسان می‌گرفتند.
   * صحنه پنج مرحله را هم‌زمان نشان می‌دهد، پس این دو می‌توانستند هم‌زمان روی
   * صفحه باشند و نتیجه‌اش این بود که با بردنِ اشاره‌گر روی کاشیِ فعلی، یک
   * کاشیِ دیگر چند قدم جلوتر هم برجسته می‌شد — و React هم دو فرزندِ هم‌کلید
   * می‌دید.
   *
   * شمارهٔ نوبت داخلِ شناسه است، پس هر نوبت یکتاست حتی اگر پرسش تکراری باشد.
   */
  uid: string;
