...
export type RawAnswer = {
   ...
   * زمانِ پاسخ (ISO).
   *
   * ⚠️ `bucketize` به آن کار ندارد و عمداً هم نباید داشته باشد — سطل‌بندی
   * کلِ تاریخ را می‌شمارد. این ستون برای `topicDeltas` است، که همین ردیف‌ها
   * را به دو پنجرهٔ زمانی می‌شکند تا بگوید «بهتر شدی یا نه». اختیاری است تا
   * منبعی که زمان ندارد بتواند در سطل‌بندی شرکت کند بی‌آنکه تاریخِ جعلی
   * بسازد.
   */
  at?: string;
};
