...
- **کپچا یک شاهد است، نه دروازه.** کاربرانِ سروا ایرانی‌اند و ویجتِ کلودفلر
  ممکن است از ایران بار نشود؛ اگر کپچا اجباری بود، همان کاربرِ واقعی نمی‌توانست
  وارد شود. پس سه حالت دارد و پیش‌فرضش `off` است — جزئیات در
  `lib/auth/captcha-policy.ts`.

- **محافظتِ واقعی به هیچ سرویسِ خارجی وصل نیست**: سقفِ نرخ به ازای
  ایمیل/شماره/IP در دیتابیس (`lib/api/rate-limit-db.ts`، ری‌استارت را تاب
  می‌آورد)، سقفِ تلاشِ حدسِ رمز و کد، argon2id با زمانِ پاسخِ یکسان برای کاربرِ
  موجود و ناموجود، و **سقفِ سراسریِ خرجِ پیامک** (`lib/sms/budget.ts`) که تنها
  عددی است که به «بدترین حالت، امشب چقدر خرج می‌شود؟» جواب می‌دهد.
...
اشتراکِ پولی. سه چیز را پیش از دست زدن به کدش بدانید:

- **یک کلید، همه‌چیز.** `تنظیمات ← سروا پلاس ← فروش و قفل‌های سروا پلاس`
  کلِ سیستم را روشن و خاموش می‌کند. خاموش یعنی سایت *دقیقاً* مثل قبل از
  پولی‌شدن: نه صفحهٔ خریدی، نه قفلی، نه نشانی در هدر. هیچ داده‌ای پاک
  نمی‌شود و با روشن‌کردن دوباره همه‌چیز برمی‌گردد. (اثرش تا حدود یک دقیقه
...
- **تنها منبعِ حقیقتِ دسترسی `plus_entitlements` است.** نه نقشِ کاربر، نه
  وجودِ یک سفارش، نه هیچ‌چیز در مرورگر. هر قفلِ سمتِ سرور از
  `requirePlus()` در `lib/plus/entitlement.ts` می‌گذرد و همان یک تابع
  جوابِ «این کاربر پلاس دارد؟» را می‌دهد.

مسیرها: `/plus` (عمومی)، `/checkout`، `/payment/return|result`،
`/panel/subscription|billing|support|analysis`، و در پنل مدیریت
`/admin/plus` و `/admin/plus/tickets`.

⚠️ **درگاه پرداخت هنوز وصل نیست.** آداپتورِ فعلی «آزمایشی» است و روی سرورِ
...
- **یافتنِ محتوا** — یک مصراع یا واژه از گزینه‌ها را بنویسید تا بگوید مالِ کدام
  ...
  > جست‌وجو **دقیقاً همان بایت‌ها** را نمی‌خواهد. اعراب، نیم‌فاصله، `ی` و `ک`
  > عربی در برابر فارسی، و حتی فاصله و نشانه‌گذاری نادیده گرفته می‌شوند. یعنی
  > یک بیتِ دو مصراعی را می‌توانید یک‌نفس بنویسید و همان گزینه پیدا می‌شود —
  > در حالی که در بانک، دو خطِ جدا از هم است.
...
«رفتن به همین مورد» شما را نه به فهرستِ بخش، بلکه به **خودِ آن محتوا** می‌برد:
یک گزارشِ عروض سماعی مستقیم فرمِ ویرایشِ همان سؤال را باز می‌کند، و در واژه‌یاب،
مدارِ دستور، جفت‌ها و نینجا پنل روی همان پایه و درس (یا نقش) باز می‌شود و ردیفِ
مربوطه چند ثانیه برجسته می‌ماند.

«پلِ وزن» پنلِ ویرایش ندارد؛ لینکش به **کنسول SQL با کوئریِ آمادهٔ همان ردیف**
می‌رود (کوئری فقط نوشته می‌شود — اجرا با دکمهٔ همیشگیِ خودتان است). دو بخش هم
اصلاً لینک نمی‌گیرند چون محتوایشان در پایگاه‌داده نیست: «کوتاه یا بلند» که از
بستهٔ داخلِ کد می‌آید، و درسنامه که فایل است.
