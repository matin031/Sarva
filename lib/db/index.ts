...
function createPool(): Pool {
  const pool = mysql.createPool({
    ...
    // ⚠️ `utf8mb4_unicode_ci` و نه `utf8mb4_0900_ai_ci`.
    //
    // این مقدار در دستِ handshake به سرور فرستاده می‌شود، پیش از هر
    // کوئری‌ای. اگر سرور آن collation را نشناسد، *اتصال* رد می‌شود — نه یک
    // کوئری، بلکه همه‌چیز. و خانوادهٔ utf8mb4_0900_* فقط در MySQL 8 هست؛
    // روی MariaDB سایت اصلاً به دیتابیس وصل نمی‌شد.
    //
    // این فقط collationِ *نشست* است و ربطی به collationِ ستون‌ها ندارد:
    // مقایسه‌ها با collationِ خودِ ستون انجام می‌شوند. کاری که این مقدار
    // می‌کند تعیینِ رمزگذاریِ رشته‌هایی است که رد و بدل می‌شوند — و
    // utf8mb4 بودنش همان چیزی است که اهمیت دارد.
    charset: "utf8mb4_unicode_ci",
  });

  // ⚠️ حالتِ سخت‌گیرانه روی *هر* اتصالِ تازهٔ pool.
  //
  // این تنظیم به سرور سپرده نمی‌شود، چون سرورها با هم فرق دارند و روی هاست
  // اشتراکی دستِ ما به my.cnf نمی‌رسد:
  //
  //   • MySQL 8 به‌طور پیش‌فرض ONLY_FULL_GROUP_BY و STRICT_TRANS_TABLES دارد.
  //   • MariaDB 10.11 به‌طور پیش‌فرض ONLY_FULL_GROUP_BY **ندارد**.
  //
  // و نبودنش دقیقاً از آن نوع تفاوت‌هایی است که خطا نمی‌دهند: یک GROUP BY
  // ناقص به‌جای رد شدن، مقدارِ دلخواهی از یکی از ردیف‌های گروه برمی‌گرداند.
  // همان کوئری روی دو سرور دو جواب می‌دهد و هیچ‌کدام شکایتی نمی‌کند.
  //
  // STRICT_TRANS_TABLES هم به همین اندازه مهم است: بدون آن، رشتهٔ بلندتر از
  // ستون بریده می‌شود و تاریخ نامعتبر صفر می‌شود، هر دو فقط با یک warning.
  //
  // رویداد `connection` روی هر اتصالِ تازه‌ای که pool می‌سازد اجرا می‌شود،
  // پس اتصال‌هایی که بعداً ساخته می‌شوند هم پوشش دارند.
  //
  // ⚠️ دو نکتهٔ ظریف در همین چند خط:
  //
  //   ۱) اتصالی که این رویداد می‌دهد، **اتصالِ callback است و نه promise** —
  //      حتی روی pool ای که از mysql2/promise ساخته شده. تایپ‌های TypeScript
  //      خلافش را می‌گویند؛ در عمل `await` رویش خطای صریح می‌دهد. پس شکل
  //      callback به کار می‌رود و تایپ هم صریح cast می‌شود.
  //
  //   ۲) منتظرش نمی‌مانیم و این درست است: MySQL دستورهای یک اتصال را
  //      به‌ترتیب اجرا می‌کند، و چون این *اولین* چیزی است که روی اتصالِ تازه
  //      صف می‌شود، پیش از هر کوئریِ برنامه اجرا می‌شود. (آزموده شد: اولین
  //      کوئری روی اتصالِ تازه sql_mode را از قبل تنظیم‌شده می‌بیند.)
  pool.on("connection", (connection) => {
    (connection as unknown as {
      ...
      (err: unknown) => {
        if (!err) return;
        logger.error("تنظیم sql_mode روی اتصال تازه شکست خورد", {
          event: "db.sql_mode.failed",
          err,
        });
      },
    );
