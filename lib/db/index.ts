 ...
 *
 * ⚠️ نسخهٔ قبلی مستقیماً `JSON.stringify(params)` را در لاگ خطا می‌نوشت. آن
 * کار برای دیباگ عالی بود و از نظر امنیتی گران: هر بار که یک کوئری خطا
 * می‌داد، این‌ها در `docker compose logs` می‌نشستند —
 *
 ...
 *
 * رمز عبورِ متن‌ساده هرگز به این لایه نمی‌رسید (همیشه قبلش argon2 می‌شود)،
 * ولی بقیه کافی بودند: یک فایل لاگِ لو رفته = دسترسی به حساب‌ها.
 *
