...
/* ────────────────────── اشتراکِ دبیرِ تأییدشده ─────────────────────────── */

/**
 * دسترسیِ دائمیِ دبیر — وقتی مدیر یک درخواستِ دبیری را تأیید می‌کند.
 *
 * ⚠️ `tx` می‌گیرد و خودش تراکنش باز نمی‌کند، بر خلافِ `manualGrant`.
 *
 * دلیلش همان استدلالِ `activateForOrder` است: تغییرِ نقش به `teacher` و
 * روشن شدنِ اشتراک باید یک اتفاق باشند. اگر جدا بودند، یک قطعیِ بینشان یعنی
 * کاربری که «دبیر» است ولی پلاس ندارد — یا برعکس، پلاسی که دلیلش در هیچ
 * ردیفی نوشته نشده و شش ماه بعد کسی نمی‌فهمد از کجا آمده.
 *
 * ⚠️ `ends_at` صریحاً NULL است: بند ۸ می‌گوید مادام‌العمر. توضیحِ اینکه چرا
 * NULL بهتر از «یک تاریخِ خیلی دور» است، بالای جدول در ۰۰۳ آمده.
 *
 * ⚠️ و بر خلافِ `activateForOrder` اینجا `nextPeriodStart` صدا زده نمی‌شود.
 * آن تابع «دورهٔ تازه را به انتهای دورهٔ فعلی بچسبان» را پیاده می‌کند، که
 * برای یک دسترسیِ *بی‌پایان* بی‌معناست: دبیری که یک اشتراکِ یک‌ماههٔ خریداری‌شده
 * دارد نباید یک ماه صبر کند تا دسترسیِ دائمی‌اش شروع شود. هر دو ردیف کنارِ
 * هم می‌مانند و `resolveCoverage` طبق قاعدهٔ همیشگی‌اش پوششِ درست را
 * می‌سازد.
 *
 * بازگشتی `false` یعنی این کاربر از قبل دسترسیِ دبیری داشته — تأییدِ دوباره
 * (مثلاً بعد از یک رد و درخواستِ تازه) نباید ردیفِ دوم بسازد.
 */
export async function grantTeacherPlus(
  tx: Tx,
  params: { userId: string; grantedBy: string; reason: string },
): Promise<{ created: boolean; entitlementId: string }> {
  const { userId, grantedBy, reason } = params;

  await lockUser(tx, userId);

  // ⚠️ فقط ردیفِ *فعالِ* دبیری حساب می‌شود. اگر مدیری روزی دسترسیِ دبیریِ
  // کسی را لغو کرده و بعد دوباره تأییدش می‌کند، باید ردیفِ تازه ساخته شود —
  // وگرنه تأیید بی‌اثر می‌ماند و هیچ خطایی هم نمی‌دهد.
  const existing = await tx.queryOne<{ id: string }>(
    `select id from plus_entitlements
      where user_id = ? and source = 'teacher_verified' and revoked_at is null
      limit 1`,
    [userId],
  );
  if (existing) return { created: false, entitlementId: existing.id };

  const id = randomUUID();

  /* ⚠️⚠️ `starts_at` از **جاوااسکریپت** می‌آید و نه از `now(6)`ِ SQL — و این
     یک باگِ واقعی را رفع می‌کند که آزمونِ روی دیتابیسِ واقعی پیدایش کرد.

     `lib/db` به درایور می‌گوید هر DATETIME را UTC بخواند (`timezone: "Z"`)،
     ولی نشستِ دیتابیس `time_zone = SYSTEM` دارد و `now(6)` وقتِ *محلیِ
     سرور* را برمی‌گرداند. روی سروری با ساعتِ ایران، این یعنی مقدارِ
     نوشته‌شده ۳ ساعت و نیم **جلوتر** از `Date.now()` خوانده می‌شود.

     پیامدش دقیقاً همین‌جا کشنده بود: `resolveCoverage` شرطِ
     `starts_at <= now` را در جاوااسکریپت می‌سنجد، پس اشتراکِ مادام‌العمرِ
     دبیری که همین حالا تأیید شده، تا سه‌ساعت‌ونیم **فعال نمی‌شد** — بدونِ
     هیچ خطایی، فقط یک دبیرِ تأییدشده که پلاس نداشت.

     دو خواهرِ این تابع (`activateForOrder` و `manualGrant`) از اول شیءِ
     `Date` پاس می‌دادند؛ این یکی تنها جایی بود که از `now(6)` استفاده
     می‌کرد. حالا هر سه یکسان‌اند و هیچ‌کدام به تنظیمِ منطقهٔ زمانیِ سرور
     وابسته نیستند.

     (خودِ آن ناسازگاریِ نشست یک مسئلهٔ عمومی‌تر است و در گزارش آمده؛ این
      تغییر فقط این مسیر را از آن مستقل می‌کند.) */
  await tx.execute(
    `insert into plus_entitlements
       (id, user_id, source, starts_at, ends_at, reason, granted_by)
     values (?, ?, 'teacher_verified', ?, null, ?, ?)`,
    [id, userId, new Date(), reason, grantedBy],
  );

  return { created: true, entitlementId: id };
}

/**
 * لغوِ اشتراکِ **دبیری** — و فقط همان.
 *
 * =============================================================================
 * ⚠️ شرطِ `source = 'teacher_verified'` قابلِ حذف نیست
 * =============================================================================
 *
 * وقتی مدیر دسترسیِ دبیریِ کسی را لغو می‌کند، چیزی که باید برود همان
 * اشتراکی است که **از همان تأیید** آمده. کاربر ممکن است هم‌زمان اشتراکِ
 * **خریداری‌شده** یا هدیهٔ دستیِ مدیر هم داشته باشد، و آن‌ها هیچ ربطی به
 * دبیری ندارند.
 *
 * بدونِ این شرط، لغوِ دسترسیِ دبیری، پولِ کاربر را هم می‌سوزاند — و چون
 * `revoked_at` برگشت‌پذیر نیست، جبرانش یک هدیهٔ دستیِ تازه لازم دارد و
 * سابقهٔ خرید هم دیگر درست نیست.
 *
 * ⚠️ و برخلافِ `revokeEntitlement` پایین، این تابع تراکنشِ خودش را باز
 * **نمی‌کند**: باید داخلِ همان تراکنشی بنشیند که نقشِ کاربر را هم عوض
 * می‌کند. یک مرز بینشان یعنی حالتِ «دیگر دبیر نیست ولی هنوز پلاسِ دبیری
 * دارد».
 *
 * ⚠️ `now(6)` و نه یک `Date` از Node — عمداً، تا با خواهرش
 * (`revokeEntitlement`) در همین ستون یکسان بماند. این مقدار فقط خوانده و
 * نمایش داده می‌شود؛ هیچ‌جا با ساعتِ جاوااسکریپت مقایسه نمی‌شود
 * (`lib/plus/entitlement.ts` فقط `revoked_at === null` را می‌سنجد)، پس
 * انتخابِ ساعت روی درستی اثری ندارد.
 *
 * ⚠️ چند ردیف ممکن است برگردد و این خطا نیست: اگر کسی در گذشته لغو و
 * دوباره تأیید شده باشد، ردیفِ قدیمی از قبل `revoked_at` دارد و شرطِ
 * `is null` کنارش می‌گذارد. عدد فقط برای گزارش است.
 */
export async function revokeTeacherPlus(
  tx: Tx,
  userId: string,
