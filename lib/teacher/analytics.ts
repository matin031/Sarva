 ...
 * | `rang_ara_answers`        | سرور (بازپخشِ ضربه‌ها)        |
  ...
  const rows = await query<{
    ...
    `select user_id,
       ...
       from (
         ...
         union all
         select user_id, answered_at, 'answer', 1, is_correct
           from rang_ara_answers where user_id in (${ph})
         union all
    ...
    `select ${tehranDay("at")} as day,
       ...
       from (
         ...
         select answered_at, is_correct from role_hunt_answers
          where user_id = ? and answered_at >= now(6) - interval ? day
         union all
         select answered_at, is_correct from rang_ara_answers
          where user_id = ? and answered_at >= now(6) - interval ? day
         union all
      ...
      group by 1
      order by 1`,
    /* ⚠️ هشت شاخه و شانزده `?`. هر `?` در MySQL پارامترِ بعدی را مصرف
