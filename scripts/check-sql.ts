  ...
  const walk = (node: ts.Node) => {
    if (ts.isCallExpression(node)) {
      ...
      if (name && SQL_CALLS.has(name) && node.arguments.length > 0) {
        ...
        } else if (ts.isIdentifier(arg) && constants.has(arg.text)) {
          /* ⚠️ کوئری‌ای که در یک ثابت نگه داشته شده و با نامش پاس می‌شود:
             `query(OVERVIEW_BY_DAY, […])`.
          
             بدونِ این شاخه، چنین فراخوانی **کاملاً نامرئی** بود — نه در
             شمارشِ کامل می‌آمد و نه در نیمه‌پویا. یعنی بررسیگر بی‌صدا
             ردش می‌کرد و خروجی‌اش «همه سالم‌اند» می‌ماند.
          
             آزموده شد: یک توکنِ عمداً خراب در چنین کوئری‌ای گذاشته شد و
             پیش از این اصلاح گزارش نشد.
          
             این الگو وقتی لازم می‌شود که *دو* شکلِ کامل از یک کوئری وجود
             دارد و کد بینشان انتخاب می‌کند — و همان‌جا بیشترین احتمالِ
             بررسی‌نشده ماندن را دارد. */
          found.push({ file, line, sql: constants.get(arg.text)! });
