          ...
          {classes.map((klass) => (
            <Card key={klass.id}>
              <CardContent className="flex flex-col gap-3">
                ...
                {/* ⚠️ حالتِ «دبیر دیگر دبیر نیست».

                    لغوِ دسترسیِ دبیری عمداً هیچ‌چیزِ کلاس را حذف نمی‌کند:
                    عضویت‌ها، بازخوردها و کلاس سرِ جایشان می‌مانند. یعنی تا
                    دیروز، از دیدِ دانش‌آموز *هیچ اتفاقی نیفتاده بود* — کارت
                    کاملاً عادی بود و او همچنان فکر می‌کرد کسی کارش را
                    می‌بیند.

                    سه چیز که این متن عمداً **نمی‌گوید**:

                      • دلیلِ لغو. آن بینِ مدیر و آن دبیر است. نوشتنش روی
                        صفحهٔ چند ده نوجوان یعنی پخشِ یک اتهام.
                      • «اخراج شد» یا هر لحنِ قضاوتی. حتی «دیگر دبیر نیست»
                        هم نه — حسابِ طرف ممکن است به هر دلیلی، از جمله
                        درخواستِ خودش، عوض شده باشد.
                      • هیچ وعده‌ای دربارهٔ آینده. نه «به‌زودی دبیرِ تازه‌ای
                        می‌آید» و نه «کلاس بسته می‌شود» — هیچ‌کدام معلوم
                        نیست.

                    و کلاس بایگانیِ دروغین نمی‌شود: `is_active` دست‌نخورده
                    است و نشان دادنش به‌عنوان بایگانی‌شده یعنی نمایشِ چیزی
                    که در دیتابیس نیست.

                    ⚠️ و `=== false` و نه `!teacherActive`: اگر روزی ردیفی
                    بدونِ این فیلد به اینجا برسد (نسخهٔ قدیمیِ کش‌شده)، شکلِ
                    نفی این هشدار را به *همه* نشان می‌داد. مقایسهٔ صریح در
                    آن حالت ساکت می‌ماند. */}
                {klass.status === "active" && klass.teacherActive === false && (
                  <div className="flex flex-col gap-1 rounded-lg border border-border bg-foreground/[0.03] px-2.5 py-2">
                    <p className="text-[12px] font-medium">
                      دبیر این کلاس در حال حاضر فعال نیست.
                    </p>
                    <p className="text-[11.5px] leading-relaxed text-muted-foreground">
                      عملکرد آموزشی جدید شما از طریق این کلاس برای دبیر قبلی قابل مشاهده
                      نیست. سوابق و بازخوردهای قبلی شما حفظ می‌شوند.
                    </p>
                  </div>
                )}
