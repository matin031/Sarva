...
import { firstNameField, lastNameField } from "./name";
...
/**
 * ⚠️ قانونِ نام به `lib/profile/name.ts` منتقل شد و اینجا فقط دوباره
 * export می‌شود — پس هر کسی که امروز از این فایل می‌خواندش، همان‌طور
 * می‌خواند. دلیلِ جابه‌جایی (و اینکه چرا مهم است) بالای همان فایل نوشته
 * شده: خلاصه‌اش اینکه این فایل `@/lib/geo` را می‌آورد و آن یک JSON
 * چهل‌وهفت‌کیلوبایتی است که صفحهٔ ورود هیچ کاری با آن ندارد.
 */
export { firstNameField, lastNameField } from "./name";
...
/**
 * شناسهٔ شهرِ ذخیره‌شده، فقط اگر هنوز در فهرست باشد — وگرنه `null`.
 *
 * =============================================================================
 * ⚠️ چرا این تابع لازم شد
 * =============================================================================
 *
 * فهرستِ مکان‌ها از «شهرستان» به «شهر» رفت (چرایی‌اش بالای `lib/geo`). قاعدهٔ
 * ساختِ آن فهرست این است که شناسهٔ قدیمی هرگز به شهرِ دیگری داده نشود — پس
 * ۸۶ شهرستان که نامشان با نامِ مرکزشان فرق دارد (لنجان → زرین‌شهر، برخوار →
 * دولت‌آباد) شناسه‌شان «یتیم» شد: در دیتابیسِ کاربران هست و در فهرست نیست.
 *
 * ⚠️ بدونِ این تابع، آن کاربر **اصلاً نمی‌توانست پروفایلش را ذخیره کند** — و
 * نه فقط شهرش را نمی‌دید:
 *
 *   ۱. فرم `user.cityId` را در state می‌گذارد.
 *   ۲. انتخابگر گزینه‌ای با آن مقدار پیدا نمی‌کند و «انتخاب کنید» نشان
 *      می‌دهد — یعنی کاربر فکر می‌کند چیزی انتخاب نشده.
 *   ۳. کاربر نام یا مدرسه‌اش را عوض می‌کند و ذخیره می‌زند. مقدارِ یتیم
 *      هنوز در state است و فرستاده می‌شود.
 *   ۴. `isValidLocation` ردش می‌کند: «شهر انتخاب‌شده در استان انتخاب‌شده
 *      نیست» — زیرِ فیلدی که *خالی به نظر می‌رسد*.
 *
 * بن‌بستِ کامل: خطا دربارهٔ فیلدی است که کاربر هیچ‌وقت پرش نکرده و پاک
 * کردنش هم کاری نمی‌کند، چون از قبل خالی دیده می‌شود.
 *
 * ⚠️ و چرا اعتبارسنجیِ سرور شل نشد: چون اگر `isValidLocation` این حالت را
 * قبول می‌کرد، هر شناسهٔ بی‌معنایی از هر مسیری قابلِ نوشتن می‌شد. مقدارِ
 * کهنه در *ورودیِ فرم* دور انداخته می‌شود و نه در دروازه.
 *
 * ⚠️ این تابع را هر جایی که یک `cityId`ِ ذخیره‌شده را داخلِ یک فرم
 * می‌گذارد باید صدا زد، نه فقط در نمایش. برای *نمایش*، `locationLabel` خودش
 * درست رفتار می‌کند و به این نیازی نیست.
 */
export function usableCityId(
  provinceId: string | null | undefined,
  cityId: string | null | undefined,
): string | null {
  if (!cityId) return null;
  return isValidLocation(provinceId, cityId) ? cityId : null;
}
