...
/**
 * چیزهایی که هرگز در زمانِ اجرا خوانده نمی‌شوند و نباید در خروجی standalone
 * بنشینند.
 *
 * ⚠️ چرا اصلاً لازم است: مسیرهایی که فایل را با مسیرِ ساخته‌شده در زمانِ اجرا
 * می‌خوانند (`/uploads` و مدارکِ دبیران)، برای ردیابِ Next مبهم‌اند. واکنشش
 * محتاطانه است: *همه‌چیز* را در خروجی می‌گذارد — tests/ و docs/ و کلِ سورسِ
 * TypeScript. حجم از ۱۲۴ مگابایت به ۱٫۲ گیگابایت می‌رفت و سورسِ کاملِ سایت
 * روی یک هاستِ اشتراکی می‌نشست.
 *
 * ⚠️⚠️ و دو الگوی آخر با یک نشتِ واقعی اضافه شدند:
 *
 * بستهٔ آمادهٔ آپلود، فایلِ `.env` *واقعی* را در خودش داشت — با گذرواژهٔ
 * دیتابیس، کلیدِ امضای ورود، pepperِ کدهای یک‌بارمصرف و کلیدِ پیامک. کنارش
 * `vocab-images.tar.gz` هم بود که ۱۱۶ مگابایت از ۳۴۶ مگابایتِ بسته را
 * می‌گرفت، بی‌آنکه هیچ‌وقت خوانده شود.
 *
 * `.env*` اینجا امن است چون خودِ برنامه روی هاست `.env` را از کنارِ
 * `app.js` می‌خواند (`cpanel-app.js`)، و آن فایل را کاربر خودش آنجا
 * می‌گذارد — نه از داخلِ بسته.
 */
const TRACING_EXCLUDES = [
  "./app/**/*",
  "./components/**/*",
  "./lib/**/*",
  "./scripts/**/*",
  ...
  //
  // بدون این خط، deploy/ (که یک zip چند صد مگابایتی دارد) در خروجی
  // standalone می‌نشست و بستهٔ بعدی آن را با خودش می‌برد — یعنی هر build
  // حجم را تصاعدی زیاد می‌کرد.
  "./deploy/**/*",
  "./cpanel-app.js",
  "*.md",
  "*.yml",
  ...
  "eslint.config.mjs",
  "postcss.config.mjs",
  // ⚠️ رازها — و یک هشدارِ صریح برای خواننده:
  //
  // **این چهار الگو امروز کار نمی‌کنند.** آزموده شد: با هر چهار شکل،
  // `.env` همچنان در `.next/standalone` می‌نشیند. ردیابِ Next با dotfile ها
  // جور دیگری رفتار می‌کند و این الگوها به آن نمی‌رسند.
  //
  // پس نگهبانِ واقعی اینجا نیست: فهرستِ `JUNK` در
  // `scripts/cpanel/build-package.mjs` است که روی *خروجیِ نهایی* کار
  // می‌کند و بعدش هم صریح بررسی می‌کند که چیزی از قلم نیفتاده باشد.
  //
  // این خطوط می‌مانند چون بی‌ضررند و اگر روزی رفتارِ ردیاب عوض شد، لایهٔ
  // اولِ دفاع را هم داریم. ولی به آن‌ها **تکیه نکنید**.
  ".env",
  ".env.*",
  "./.env",
  "./.env.*",
  // آرشیوهای محتوا — دستی روی هاست باز می‌شوند و در اجرا خوانده نمی‌شوند.
  "*.tar.gz",
];
...
const nextConfig: NextConfig = {
  ...
  /**
   * ⚠️ بدون این، خروجی standalone کلِ پروژه را با خودش می‌برد.
   *
   * مسیر /uploads فایل را با `createReadStream(target)` می‌خواند و `target`
   * در زمانِ اجرا ساخته می‌شود. ردیابِ Next نمی‌تواند بداند آن مسیر کجاست،
   * پس محتاطانه *همه‌چیز* را در خروجی می‌گذارد: tests/ و docs/ و کلِ سورسِ
   * TypeScript.
   *
   * دو ضرر داشت و هر دو واقعی‌اند:
   *
   *   • حجم آپلود به هاست از ۱۲۴ مگابایت به ۱٫۲ گیگابایت می‌رفت.
   *   • سورسِ کامل سایت — از جمله منطق احراز هویت — روی یک هاست اشتراکی
   *     می‌نشست، بی‌آنکه هیچ لزومی داشته باشد.
   *
   * چیزهای زیر هیچ‌کدام در زمانِ اجرا خوانده نمی‌شوند: کدِ TypeScript از
   * پیش به .next/server کامپایل شده و migration ها با ابزارِ جدا اجرا
   * می‌شوند، نه از داخل اپ.
   */
  //
  // ⚠️ کلید یک glob است که با *مسیرِ route* مطابقت داده می‌شود، نه نامِ فایل.
  // نسخهٔ اول اینجا `"/uploads/[...path]"` نوشته شده بود و بی‌صدا هیچ کاری
  // نمی‌کرد: براکت‌ها برای picomatch یعنی character class، پس آن الگو با
  // هیچ مسیری نمی‌خورْد. `/uploads/**` هم ساده‌تر است و هم واقعاً می‌خورد.
  outputFileTracingExcludes: {
    // ⚠️ همان فهرست برای *هر سه* گروهِ مسیری که فایل را پویا می‌خوانند.
    //
    // تا دیروز فقط `/uploads/**` اینجا بود و کافی بود. با آمدنِ مدارکِ
    // دبیران، دو مسیرِ تازه هم `createReadStream`/`stat` با مسیرِ زمانِ
    // اجرا دارند — و ردیابِ Next برای هرکدام جداگانه تصمیم می‌گیرد. یعنی
    // نبودنشان در این فهرست، همان «کلِ پروژه را ببر» را از درِ پشتی
    // برمی‌گرداند، حتی با وجودِ خطِ `/uploads/**`.
    //
    // (در گزارشِ build هر سه با نامِ خودشان هشدار می‌دهند؛ اگر روزی مسیرِ
    //  چهارمی اضافه شد، همان هشدار پیدایش می‌کند.)
    "/uploads/**": TRACING_EXCLUDES,
    "/api/v1/teacher/**": TRACING_EXCLUDES,
    "/api/v1/admin/**": TRACING_EXCLUDES,
  },
