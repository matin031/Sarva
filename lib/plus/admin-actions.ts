  ...
  const created = await transaction(async (tx) => {
    ...
    await tx.execute(
      `insert into plus_plan_versions
         (id, plan_id, version, title, duration_days, amount_rials, is_sellable, note, created_by)
       values (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [id, planId, version, plan.title, plan.duration_days, amountRials, makeSellable,
       text(input.note, 300) || null, admin.id],
    );
    return { id, version };
  });
