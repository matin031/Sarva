 ...
 * ── وقتی بار نشد ───────────────────────────────────────────────────────────
 *
 * ⚠️ اسکریپت از `challenges.cloudflare.com` می‌آید و برای کاربرِ ایرانی
 * ممکن است اصلاً در دسترس نباشد. در آن حالت این کامپوننت `null` می‌دهد و
 * فرم **باید کار کند** — سرور در حالتِ پیش‌فرض (`optional`) نبودِ توکن را
 * مانع نمی‌داند. چرایی‌اش در `lib/auth/captcha-policy.ts` نوشته شده.
 *
 * پس پیامِ شکست هم عوض شد: تا دیروز می‌گفت «اتصال اینترنت را بررسی کنید و
 * صفحه را تازه کنید» — یعنی کاربر را می‌فرستاد سراغ کاری که نه لازم بود و
 * نه کمکی می‌کرد، و این حس را می‌داد که فرم کار نخواهد کرد. حالا فقط
 * می‌گوید بی‌اشکال است و ادامه بدهد.
 */
    ...
    <div className="flex flex-col gap-2">
      ...
      {failed && (
        <p className="text-xs text-muted-foreground">
          تأیید امنیتی بارگذاری نشد. می‌توانی ادامه بدهی.
        </p>
      )}
    </div>
  );
