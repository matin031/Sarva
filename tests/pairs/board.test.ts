 ...
 *
 *      ⚠️ این قاعده برای دو زمین دو شکل دارد. صفحهٔ بزرگ `grid` است و
 *      ردیفِ ناقصش یک خانهٔ خالیِ آشکار، پس آنجا ردیف‌ها باید **پر**
 *      باشند. موبایل `flex` با `justify-center` است و ردیفِ کوتاهش
 *      وسط‌چین می‌شود، پس آنجا فقط ردیفِ **تک‌کارته** ممنوع است.
 *
...
/**
 * قاعدهٔ زمینِ موبایل — سست‌تر از `tidy` و عمداً.
 *
 * ⚠️ موبایل از این به بعد اجازهٔ ردیفِ ناتمام دارد: زمینش `flex` با
 * `justify-center` است و خطِ کوتاهِ وسط‌چین ایراد ندارد. ولی **کارتِ تنها**
 * همچنان ممنوع است — یک کارت در ردیفِ آخر، حتی وسط‌چین، شبیه اشتباه است.
 * (چراییِ کامل بالای `mobileColumns` در `lib/literary-pairs.ts`.)
 */
const noOrphan = (cards: number, cols: number) => cards <= cols || cards % cols !== 1;
...
describe("زمینِ جفت‌های ادبی", () => {
  test("هیچ دستی کارتِ تنها در ردیف آخر نمی‌گذارد", () => {
    for (let total = 1; total <= 60; total++) {
      for (const size of memoryRoundSizes(total)) {
        ...
        assert.ok(
          noOrphan(cards, base),
          `${total} جفت → دستِ ${size}تایی در ${base} ستونِ موبایل کارتِ تنها می‌گذارد`,
        );
        /* ⚠️ دو ستون دیگر هرگز نباید برگردد: همان چیزی بود که دستِ
           پنج‌جفتی را روی موبایل به پنج ردیف و هزار پیکسل اسکرول
           می‌رساند. */
        assert.ok(
          base === 3 || base === 4,
          `${total} جفت → دستِ ${size}تایی روی موبایل ${base} ستون گرفت`,
        );
      }
    }
  });
  ...
  test("فهرستِ پیش‌فرضِ داخلِ کد هم زمینِ درستی می‌سازد", () => {
    for (const round of buildMemoryRounds(LITERARY_PAIRS)) {
      ...
      assert.ok(noOrphan(deck.length, base));
    }
  });
});
